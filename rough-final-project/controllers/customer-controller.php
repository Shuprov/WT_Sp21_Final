...
function insertCustomer($name,$username,$password,$email)
{
	$query="insert into customer values(NULL,'$name','$username','$password','$email')";
	execute($query);
	header("Location: all-customers.php");
}
function updateCustomer($id,$name,$username,$password,$email)
{
	$query="update customer set name='$name',username='$username',password='$password',email=   '$email' where id='$id'";
	execute($query);
	header("Location: all-customers.php");
}
