...
//validation
            ...
            $y= strpos($_POST["pass"],"?");
             ...
             if(!$hasError){
			insertEmployee($_POST["name"],$_POST["uname"],$_POST["pass"],$_POST["email"]);
...
function insertEmployee($name,$username,$password,$email)
{
	$query="insert into employee values(NULL,'$name','$username','$password','$email')";
	execute($query);
	header("Location: all-employees.php");
}
function updateEmployee($id,$name,$username,$password,$email)
{
	$query="update employee set name='$name',username='$username',password='$password',email=   '$email' where id='$id'";
	execute($query);
	header("Location: all-employees.php");
}
